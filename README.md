...
4. **Upload the `.aab` to Play by hand.** It is the one artifact deliberately kept out of the release: an `.aab` is not installable — it is the archive Play opens to build a per-device APK — so on a download page it is a trap. Find it under the run's artifacts, as `play-bundle`. There is no service-account automation.
